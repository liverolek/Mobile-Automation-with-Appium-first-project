describe('iOS Find Element', () => {
    ...
    it.only('Exercise 1', async () => {

        const typeSearchField = 'type == "XCUIElementTypeSearchField"';

        await $('~Search').click();
        await $('~Default').click();
        await $(`-ios predicate string:${typeSearchField}`).addValue('I really enjoy this course!');
        await expect($(`-ios predicate string:${typeSearchField}`)).toHaveAttr("value");
        await $('~Clear text').click();
        await expect($(`-ios predicate string:${typeSearchField}`)).not.toHaveAttr("value");
